...
- Two guards back that up, both worth turning on before you paste a token in.
  `npm run secrets:hook` installs a pre-commit hook that refuses a commit
  containing a credential file or a filled-in `.env.example`; the same check
  runs in CI as `npm run secrets:check`. The hook matters most, because CI can
  only tell you about a secret that has already been pushed, and at that point
  the only real remedy is to rotate it.
