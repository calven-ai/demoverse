...
- Use AskUserQuestion for each interview topic (Step 1) — one topic per
...
- If `config/world.yaml` exists on entry, STOP and ask: amend the existing
  world, or reset (`npm run init -- --force` — explain it clears the local
  world and derived state, and print its external-purge guidance).
...
- `prose.yaml`: default to the shipped generic banks, then offer to tailor
  `narrative_angles` and `objection_themes` to the invented product (recommended
  — 10 minutes of tailoring is the single biggest realism win).
...
single artifact — it doubles as showing the user what the prompts look like),
then `npm run apply -- --ingest` and `npm run lint`. Show the user the filed
