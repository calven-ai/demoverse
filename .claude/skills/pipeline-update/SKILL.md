...
generates its *entire* sales cycle at once. That is the historical seed motion.
This one adds a thin slice of new life to *many* deals. Never generate a full
detail layer for a deal here. A deal born this week has had exactly one
discovery call, and maybe an intro email. Its Evaluation call has not happened
yet. It happens in a later increment, when the deal reaches Evaluation.
...
3. **Ingest.** `npm run apply -- --ingest`. Anything that fails validation stays
   ...
   only those artifacts, then ingest again.
